...
> 本章讨论面向智能体的高级推理方法，重点是多步逻辑推断与问题求解。这类技术不再满足于简单的顺序处理，而是把智能体的内部推理过程显式化，便于它拆解问题、权衡中间步骤，并得出更稳健、更准确的结论。它们有一个共同点：在推理阶段投入更多计算资源，为智能体或其底层 LLM 提供更长的处理时间或更多推理步数。与「一口气答完」相比，这种做法允许智能体迭代改进、探索多条路径，或调用外部工具。对需要深入分析和反复斟酌的复杂问题来说，这通常能明显提升准确性、连贯性和稳健性。
...
> **思维链（Chain-of-Thought, CoT）** 提示通过模仿人类逐步思考，显著增强了 LLM 的复杂推理能力（见图 1）。CoT 不直接给答案，而是引导模型先产出中间推理步骤，把难题拆成更小、更易处理的子问题，因此在算术、常识推理、符号操作等多步任务上往往表现更好。它的一大优势，是把「一步难解」变成「多步可解」，从而提升推理透明度，既有助于提高准确率，也便于理解决策过程和排查错误。实现方式很灵活：可以提供 few-shot 的逐步示范，也可以直接要求模型「一步一步想」。思维链之所以有效，在于它把模型的内部处理引导到更有意识、更有逻辑的推进方式上。如今，思维链已成为当代 LLM 高级推理的基石；对自主智能体尤其关键，因为它能在复杂环境中带来更可靠、也更可审计的行为。
...
> 下面举个例子：先用一组说明规定 AI 如何思考，也就是先设定角色，再给出清晰的五步流程。这就是启动结构化推理的提示。
...
> 接着，示例展示了 CoT 如何落地：标注为「Agent's Thought Process」的部分，是模型按步骤展开的内心独白，也就是字面意义上的「思维链」；而「Agent's Final Answer」则是经过逐步推理后整理出的最终回答。
...
> **思维树（Tree-of-Thought, ToT）** 是在 CoT 之上进一步发展出来的方法。它允许 LLM 在中间步骤处分叉，探索多条推理路径，形成树状结构（见图 2）。借助回溯、自我纠正和对备选方案的搜索，它更适合处理复杂问题：模型可以在给出终稿前比较多种推理轨迹，因此在需要战略规划与决策的任务上往往表现更好。
...
> **自我纠正（Self-correction）** 也叫自我精炼，是智能体推理中的关键环节，在 CoT 中尤为常见。智能体会回头审视自己已经生成的内容和中间思考，识别歧义、信息缺口，以及理解或解答中的偏差；再通过「审阅 - 改进」的循环调整策略、提升回答质量，并在交付前尽量保证准确和完整。这种内部审视有助于得到更可靠、更高质量的输出；更多示例可见第 4 章。
...
> 这个例子展示了一套可复用的自我纠正流程，对打磨 AI 生成内容尤其有效：先起草，再对照原始要求审阅，接着落实具体修改，并反复迭代。文中先定义了「Self-Correction Agent」以及五步分析 - 修订工作流；再给出一则较弱的社交媒体草稿「Initial Draft」；重点则落在「Self-Correction Agent's Thought Process」上，也就是它如何按指令识别问题，例如互动感不足、行动号召含糊，并提出具体改进建议，如使用更有力的动词或适当加入表情符号。最后，示例给出吸收这些修改后的「Final Revised Content」。
...
> 归根结底，这项技术是把质量控制直接嵌进智能体的内容生成链路里，从而产出更精炼、更准确、质量更高的结果，也更能满足复杂场景下的用户需求。
...
> **程序辅助语言模型（PALMs）** 把 LLM 与符号推理结合起来。解题时，模型可以生成并执行 Python 等代码，把繁重计算、逻辑运算和数据处理交给确定性运行时，弥补纯语言模型在数值精度和行为一致性上的短板。遇到符号化任务时，它可以先写代码、执行，再把结果转回自然语言。这种混合范式把语义理解与精确计算结合起来，拓宽了模型可解的问题类型，也有望提高可靠性和正确率。对智能体来说，这意味着它不只是「会写会说」，还可以借助代码完成更稳妥的动作；谷歌 ADK 中通过外部工具生成代码，就是典型用法之一。
...
> **可验证奖励的强化学习（RLVR）：** 许多 LLM 使用的标准 CoT 虽然有效，但总体上仍比较朴素：它往往只沿着一条既定思路前进，很少根据题目难度动态调整。为突破这一限制，业界开始发展专门的「推理模型」：它们在给出答案前会分配可变的「思考」预算，生成更长、也更灵活的思维链（可达数千 token），从而支持自我纠正、回溯等更复杂的行为，并在更难的问题上自动投入更多计算。其背后的关键训练范式之一就是 RLVR：在答案可核对的问题上，例如数学题和代码题，通过试错学习，把长程推理能力训练出来，使模型即使没有逐条人工监督，也能逐步演化出更好的解题策略。这类模型不仅输出答案，还会留下体现规划、监控与评估能力的「推理轨迹」；而更强的推理和策略水平，正是打造「在较少人工介入下就能拆解并完成复杂任务」的自主智能体的基础。
...
> **ReAct**（Reasoning and Acting，见图 3；KB 指知识库）把 CoT 与「通过工具同外部环境交互」的能力结合起来。它不同于只负责生成最终答案的模型，而是要求智能体先想清楚下一步应该做什么：推理阶段类似 CoT 的内部规划，用于选定下一步、盘点可用工具并预判后果；随后再真正执行工具或函数调用，例如查询知识库、进行计算或调用 API。
...
> ReAct 以交错方式运行：先行动，再接收环境反馈，再把观察结果写回下一轮推理。「思考 → 行动 → 观察 → 再思考……」这一闭环，使智能体能够动态调整计划、纠正偏差，并完成那些需要与环境多轮交互的目标。相比线性的 CoT，它更能适应真实世界中的变化，因为每一步都能得到实时反馈。通过把语言模型的理解与生成能力和工具调用结合起来，ReAct 特别适合那些既需要推理、又需要执行的复杂任务。它的重要性在于，智能体不再停留在纯推理层面，而是能够在动态环境中逐步完成实际任务。
...
> **CoD（Chain of Debates，辩论链）** 是微软提出的一种正式框架。它让多个不同模型通过协作辩论来共同求解问题，从而突破单模型、单条「思维链」的局限。其运作方式有点像 AI 理事会：各方先提出观点，再对彼此的推理链进行质询与反驳；通过集体智能提高准确率、降低偏见，并改善最终答案的质量。它很像 AI 版的同行评审，能够保留可追溯、可核验的推理过程，也意味着系统正从「一个智能体给出答案」走向「多个智能体共同打磨更稳健、更可验证的结论」。
...
> **GoD（Graph of Debates，辩论图）** 则更进一步。它把讨论建模成一张动态、非线性的图，而不是一条直线。每个论点是一个节点，边表示「支持」「反驳」等关系，更贴近真实辩论中多线并进的样子；新的追问可以随时分叉、各自演进，甚至之后再汇合。最终结论不一定出现在某条链的末尾，而是从整张图里挑出证据最充分、支撑最强的那一簇论点。所谓「支撑充分」，意味着这些知识站得住脚、也能被核对：可以是广泛接受的 ground truth、经过搜索 grounding 与外部来源交叉验证的事实，也可以是多模型辩论后形成的高共识、高置信信息。相比单链叙事，这种方式更贴近复杂的真实讨论，也为集体推理提供了更扎实的事实底座。
...
> **MASS（可选进阶）：** 如果进一步研究多智能体系统（MAS）的设计，就会发现，成败很大程度上取决于两件事：给每个智能体写的提示是否合适，以及它们之间如何连接、如何传递任务（拓扑）。但这两个维度的设计空间都非常大，而且彼此耦合，手工试错很难穷尽。为了解决这个问题，研究者提出了多智能体系统搜索（MASS），用自动化搜索同时优化提示与拓扑。
...
> MASS 采用多阶段策略，在庞大的设计空间中交替优化「提示怎么写」和「智能体怎么连」（见图 4），从而系统性地推进搜索。
...
> **1. 块级提示优化：** 第一步，是按「智能体类型 / 功能块」分别优化提示，使每个模块在接入大系统前就具备独立完成职责的能力。这一步非常关键，否则后续调整拓扑，只会放大局部配置不佳的问题。以 HotpotQA 为例，「Debator」智能体的提示可以被优化为「主流媒体的事实核查专家」：负责逐条核对其他智能体给出的答案，对照原文段落，识别不一致或缺乏依据的表述。通过块级搜索得到的这类角色化提示，可以让「辩论 / 挑错」模块在尚未嵌入完整流水线前，就具备较强的信息整合能力。
...
> **2. 工作流拓扑优化：** 当块级提示稳定后，MASS 会在可配置的设计空间中挑选、排列智能体之间的连边与调用顺序，专门优化工作流拓扑。为了控制搜索成本，它会采用「影响加权」思路：相对某个基线智能体，估计每种拓扑带来的边际收益，再用分数把搜索引向更有前景的组合。以 MBPP 编程题为例，搜索到的最优形态往往不是「一条直线」，而是「多轮自我修订 + 外部工具」的混合体：一个负责写代码并进行反思的预测智能体，配合一个在测试用例上实际执行代码的执行智能体。这说明在编码场景下，把迭代式自我纠正与外部执行验证结合起来，通常优于结构更简单的 MAS。
...
> **3. 工作流级提示优化：** 最后一轮，会把整套提示当成一个整体来调。此时拓扑已经确定，接下来要做的，是让各个智能体的措辞彼此咬合，并减少隐性冲突。以 DROP 为例，最终优化会重点打磨「Predictor」的提示：先交代数据集特点（抽取式问答、偏重数值），再塞入高质量 few-shot，最后用强情境锚定职责，例如：「你是为突发新闻直播提取关键数字的专员，准确和速度同样重要」。把元知识、示范和角色设定叠在一起，针对整条流水线做联合微调，目的是进一步提高端到端准确率。
...
> 在梳理完这些推理技术之后，还需要记住一条与性能直接相关的规律：LLM 的推理扩展定律指出，在推理阶段投入更多计算资源（如更多步骤、更多样本、更复杂的解码策略等），性能通常会呈现可预期的提升。Deep Research 类产品就是很好的例子：智能体会把大课题拆成子问题，反复进行网络检索、交叉比对和综合分析，本质上都是在投入更充裕的推理预算。
...
> **深度研究（Deep Research）** 泛指一类「像不知疲倦、又有章法的助理」的智能体工具，专门替你做多轮检索和写作。代表性产品包括 Perplexity AI、Google Gemini 的深度研究能力，以及 ChatGPT 中的高阶研究模式等（见图 5）。
...
> 它们真正改变的是「搜索」这件事本身的形态：传统搜索会很快返回一串链接，后续的综合与取舍仍然要靠你自己；而深度研究则反过来。你只需要抛出一个复杂问题，并愿意给系统几分钟「时间预算」，换来的则是一份已经帮你读完、比对完、写好的长报告。
...
> 在等待的这段时间里，AI 会以智能体方式替你完成一整套对人来说极其耗时的流程：
...
> 这种流水线式方法更容易产出覆盖面更广、论证也更充分的回答，从而同时提升信息收集的效率和深度，也更贴近「智能体化」的决策辅助。
...
> 这条规律描述的是：在 **推理** 阶段多投入计算，会如何改变 LLM 的表现。它和我们更常听到的「训练扩展定律」并不是一回事。训练扩展定律讨论的是数据、参数、训练算力与模型质量之间的关系；而推理扩展定律关注的，则是模型 **已经部署完成之后**，在真正为用户生成答案时，算力与时间预算之间如何权衡。
...
> 其中一个关键洞见是：你不一定非要换更大的模型。只要在推理时愿意投入更多算力，较小的 LLM 有时也能反超。「多给算力」并不一定等于换更贵的 GPU，更常见的是采用更费时的解码策略，例如并行生成多份答案（多样化束搜索、自洽投票等），再用规则或裁判模型挑出最优结果。多跑几轮、多采样几次，确实会吃掉更多 token 和时延，但往往也能明显拉高最终回答的质量。
...
> 这为部署智能体时的成本 - 效果权衡提供了清晰框架，也打破了「参数越大就一定越强」的简单直觉：小模型如果在推理端获得更充裕的「思考预算」，有时可能超过只依赖单次生成的大模型。这里的「思考预算」泛指额外的推理步数、更复杂的搜索或验证流程，使小模型在作出最终判断前，能够探索更多路径并做更多自检。
...
> 因此，推理扩展定律是搭建「又省又好用」的智能体系统时的重要底层逻辑之一。它迫使你在以下几件事之间做更精细的取舍：
...
> 真正理解这条定律后，团队就能根据业务场景判断：算力究竟该投入训练、模型规模，还是推理阶段的多步思考流程。这样，有限预算才能被用在最能提升输出质量与实用性的地方，而不是一味追求更大的参数规模。
...
> Google 开源的 DeepSearch 示例位于 `gemini-fullstack-langgraph-quickstart` 仓库（图 6）。它提供了一套模板，演示如何基于 Gemini 2.5 与 LangGraph 搭建一个「前端 + 智能体后端」的全栈应用；既便于快速试验智能体架构，也可以对接 Gemma 等本地模型。项目通过 Docker 和分层目录脚手架降低上手成本，但它的定位仍然是 **结构清晰的参考实现**，不建议直接当作生产后端照搬。
...
> 仓库里是一个 React + LangGraph 的全栈样例，面向「高阶检索 + 对话式研究」场景。图中的智能体会调用 Gemini 动态改写检索 query，经 Google Search API 拉取网页内容；再通过反思节点找出知识空洞，多轮补搜，最后拼出带来源引用的答案。前后端都支持热更新，代码分为 `frontend/` 与 `backend/`。本地开发需要 Node.js、npm、Python 3.8+ 以及 Gemini API Key（写在 backend 的 `.env` 中）；依赖分别通过 `pip install .` 与 `npm install` 安装，也可以用 `make dev` 一键同时启动前后端。智能体拓扑定义在 `backend/src/agent/graph.py` 中，从生成查询、联网检索、反思评估，到必要时回到检索节点或进入终稿节点。若要模拟生产形态，后端会托管打包后的静态前端，并依赖 Redis（流式输出）与 Postgres（状态 / 数据）；`docker-compose up` 可起全套服务，示例 `docker-compose.yml` 里还预留了 LangSmith Key。技术栈包括 React、Vite、Tailwind、Shadcn UI、LangGraph 和 Gemini；许可证为 Apache 2.0。
...
> 概括来说，智能体的「思考」是一套把推理与行动绑在一起的结构化方法：先把步骤想清楚，边执行边检查进度，需要时再调用外部工具补信息。
...
> 这套机制的底层，通常是一款能力足够强的 LLM。它会持续生成内部推理内容，用来指导下一步动作。其经典节奏就是思考 → 行动 → 观察，循环往复：
...
> 1. **思考：** 先把当前局面用文字捋一遍，例如拆题、列计划或评估风险；这段独白既是给人类看的审计日志，也方便后续继续用提示纠偏。
> 2. **行动：** 在有限的离散动作集合中选一个执行，例如搜索、打开某个 URL、调用内部工具，或直接输出最终答案。
> 3. **观察：** 环境把结果反馈回来，可能是 SERP 摘要、页面正文，也可能是工具返回的结构化 JSON。
...
> 如此循环，观察结果会不断改写下一轮思考，直到模型判断「可以收束」并触发终止动作。
...
> 这种方法的效果，高度依赖底座模型的规划能力和常识推理能力。ReAct 论文中通常会搭配 few-shot 示例，把「人类如何边思考边行动」的轨迹示范给模型，教它在相似任务上复用同样的节奏。
...
> 「每一步是否都要显式写出思考」也可以根据任务来调节。像事实核查这类知识密集型工作，通常会在每次工具调用前后都写一段推理，以保证证据链不断；而在需要连续执行上百步的模拟环境中，则可以只在关键分叉处显式推理，把更多 token 预算留给行动本身。
...
> **是什么：** 许多复杂问题并不能靠「一句话」解决，这正是智能体面临的核心挑战：它们往往需要逻辑推断、任务分解和战略规划等多步流程。如果缺少结构化推理，智能体就很容易遗漏细节，进而给出片面甚至错误的结论。这些高级方法的核心，就是把内部「思考」过程显式化，让系统能够按步骤、有条理地处理复杂问题。
...
> **为什么：** 业界逐渐形成的一套「标准方法」，就是通过多种推理技术为智能体搭建结构化支架：CoT 与 ToT 帮助模型拆解问题、探索分支；自我纠正负责反复打磨答案；ReAct 则把推理与工具调用紧密结合，让智能体能够边检索边调整计划。把显式推理、搜索式探索、持续修正和工具使用结合起来，系统通常会更稳健、更具可解释性，也更能胜任真实任务。
...
> * 把推理过程写出来，智能体才能形成可审计的多步计划，这是自主决策与用户信任的底座。
> * ReAct 给出了「想 - 做 - 看」的主循环，让智能体走出纯文本推理，真正与环境和工具交互并不断纠偏。
> * 推理扩展定律提醒我们：性能不只取决于模型规模，还取决于系统愿意为推理投入多少时间和算力，从而换取更审慎、更高质量的行动。
> * CoT 像智能体的自言自语，把宏大目标拆成一串可执行的小动作。
> * ToT 与自我纠正提供了「多想几条路、走错了能退回来」的能力，让模型在真正动手前先把方案打磨好。
> * CoD 一类协作框架意味着系统正从单体智能体走向团队协作：多个智能体通过相互质询和交叉验证，共同处理更复杂的问题，并降低个体偏见。
> * Deep Research 等产品证明，当上述技术叠在一起时，智能体已经可以长时间、全自动地完成深度调研类任务。
> * MASS 这类搜索框架则在系统层面回答「提示怎么写、拓扑怎么连」这一问题，用算法替人完成 MAS 级别的超参搜索。
> * 只有综合使用这些推理手段，智能体才能从「会跑的脚本」进化为「少盯也能托付的自主系统」。
...
> 当代 AI 正在从「问一句答一句的工具」转向能够处理复杂目标的自主智能体。起点往往是 CoT 式的内心独白：先想清楚，再动手。但要真正称得上自主，还必须学会自我怀疑、学会改方案。自我纠正与 ToT 让智能体能够在多种策略之间比较、试错和回滚；而真正的分水岭则是 ReAct：它要求智能体不仅要想，还要能调用工具、读取环境，并在「思考 - 行动 - 观察」的闭环中实时改写计划。
...
> 更深层次的审慎推理，通常建立在推理扩展定律所允许的额外计算预算之上：增加的「思考时间」往往能转化为更稳健的自主行为。再往前一步，就是多智能体协作阶段。CoD 等框架让多个智能体像圆桌讨论一样相互质证；Deep Research 也已经证明，这种能力完全可以落地，用来替用户完成小型研究项目。工程上的终极目标，是构建可审计、可托付的自主系统：即使人类不持续在环监督，它也能把复杂问题拆解、执行并复盘。只有把显式推理与现实世界中的行动能力真正结合起来，AI 才能发展为面向复杂问题求解的智能体系统。
